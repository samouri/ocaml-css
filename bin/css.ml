...
let output_of_str o =
  ...
  | s ->
      raise
        (ArgException (Printf.sprintf "invalid string for output type: %s" s))

let output =
  let doc =
    "Pick what ocaml-css should do: pretty print, astprint, or check for errors"
  in
...
let info =
  ...
  let man =
    [ `S Manpage.s_bugs
    ; `P
        "Open bug reports as issues on the GitHub repo at \
...
let run file output_str =
  ...
  let output_flag = output_of_str output_str in
  (* learn how to make the proper CMDLiner converter for this *)
  let output =
    if output_flag = Pretty then print parsed else astPrint parsed
  in
