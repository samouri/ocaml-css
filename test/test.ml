...
let dir_contents dir = Sys.readdir dir |> Array.to_list
...
let rec printlst = function
  | [] -> ()
  | hd :: tl -> print_endline hd ; printlst tl
...
let get_test_for_dir dir =
  ...
  let expectedAst =
    Yojson.Safe.prettify (load_file (test_path ^ "/" ^ dir ^ "/ast.json"))
  in
  dir
  >:: fun _ ->
