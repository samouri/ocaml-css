...
let getRulesetItemPos (t : Types.declaration orComment) =
  match t with Comment {pos} | Else {pos} -> pos

let getStyleSheetItemPos (t : Types.rule) =
  match t with Comment {pos} | AtRule {pos} | StyleRule {pos} -> pos
...
let insertComment (stylesheet : Types.stylesheet) (comment : Types.comment) =
  ...
  | Some node ->
      ...
      if isAfter cEndP endPos then prevNodes @ (Comment comment :: postNodes)
      else
        (* mid ruleset case*)
        let newNode =
          match node with
          (*TODO: atrules should handle comments *)
          | AtRule _ | Comment _ ->
              failwith "should never see nested comments in css"
          | StyleRule {prelude; declarations; pos} ->
              let rulesIsBeforeComments (rule : Types.declaration orComment) =
                let startPos, _ =
                  match rule with Else {pos} -> pos | Comment {pos} -> pos
                in
                isBefore startPos cStartP
              in
              let prevRules, postRules =
                List.partition rulesIsBeforeComments declarations
              in
              { prelude
              ; declarations= prevRules @ (Comment comment :: postRules)
              ; pos }
        in
...
let parse ?(fp = "") (source : string) =
  ...
  with exn ->
    ...
    print_endline
      (Printf.sprintf "Error at location (%d:%d) with token: %s!" line cnum tok) ;
    raise exn
